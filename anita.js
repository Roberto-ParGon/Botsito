...
var prefix = '.' //prefijo
var NombreBot = 'Felipe Bot' // nombre del bot 
...
const banner = cfonts.render(('Felipe|Bot'), {
font : "block",
align: "center",
colors: ["blue","white"]
})
...
const enviarImagen = (imagen) => {
  anita.sendMessage(from,{image : imagen},
    {quoted, doc})
}

const enviarImagenCap = (imagen, caption) => {
  anita.sendMessage(from,{image : imagen, caption : caption},
    {quoted, doc})
}

const enviarAudio = (mensajeAudio) => {
  anita.sendMessage(from,{audioMessage : mensajeAudio},
    {doc})
}
...
case "Hola":
enviarTexto('Hola como tas')
break

case "Foto":
const sendImg = fs.readFileSync('./archivos/fotos/img1.jpg')
enviarImagen(sendImg)
break

case "FotoDes":
const sendImgDes = fs.readFileSync('./archivos/fotos/img2.jpg')
const des = 'Bienvenido al grupo'
enviarImagen(sendImg, des)
break

case 'piropos':
const random = ['Te quiero','Te odio','Chinga tu madre']
const random2 = Math.floor(Math.random()*random.length)
const randomPiro = random[random2]
enviarTexto(randomPiro)
break
